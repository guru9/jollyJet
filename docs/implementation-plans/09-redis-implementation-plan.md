...
- **Interface Methods:**
  - `get(key: string): Promise<string | null>` - Retrieves a value from Redis cache by key
  - `set(key: string, value: string, ttl?: number): Promise<void>` - Stores a value in Redis cache with optional TTL
  - `delete(key: string): Promise<void>` - Removes a key from Redis cache
  - `keys(pattern: string): Promise<string[]>` - Finds all keys matching a specific pattern
  - `flush(): Promise<void>` - Clears the entire Redis cache
  - `increment(key: string): Promise<number>` - Atomically increments a numeric value in cache
  - `setWithExpiration(key: string, ttl: number): Promise<void>` - Sets a key with automatic expiration
  - `acquireLock(key: string, ttl: number): Promise<boolean>` - Acquires a distributed lock for cache consistency
  - `releaseLock(key: string): Promise<void>` - Releases a previously acquired lock
  - `getClient(): Redis` - Gets the underlying Redis client instance
  - `isConnected(): boolean` - Checks if Redis connection is active
