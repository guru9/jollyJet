...
This document outlines the unit tests for the `Product` entity, focusing on the `isWishlistStatus` and `wishlistCount` properties.
...
- **Input**:
  ...
  const product = new Product(productProps);
  ```
- **Expected Output**: `product.isWishlistStatus` should return `false`.

#### Test Case 3: Default Value

- **Description**: Tests that the `isWishlistStatus` property defaults to `false` when not provided.
- **Input**:
  ```typescript
  const productProps = {
    name: 'Test Product',
